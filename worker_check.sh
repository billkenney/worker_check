...
if [[ "$worker_status" = 'false' ]] ; then
  ...
  #   echo "cron script is still unable to check the online status of worker at `date '+%Y-%m-%d_%H:%M:%S'`" >> /home/user/worker_check.log
  #   curl -X POST -H "Content-Type: application/json" -d '{"value1":"worker is unable to check its online status using the hiveos api"}' https://maker.ifttt.com/trigger/notify/with/key/{$iftttkey}
  # fi
fi
